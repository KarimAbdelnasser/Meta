...
router.get("/search/:email", auth, async (req, res) => {
    ...
    return res
        .status(200)
        .send(
            `You can add '${user.email}' as a friend by clicking this URL-->https://somedia--app.herokuapp.com/users/addFriend/${user._id}`
        );
});
        ...
        if (type == "fullName") {
            ...
            if (!user) {
                return res
                    .status(404)
                    .send(
                        "The user with the given email haven't registered yet!"
                    );
            }
            return res
                .status(200)
...
router.post("/addFriend/:user", auth, async (req, res) => {
    ...
    if (notification) {
        if (notification.add_friend.find((ele) => ele.friendId == userId)) {
            return res.send(
                "You have sent a friend request to this user already!"
            );
        }
        const user = await User.findById(userId);
        const form = {
            ...
            msg: `You have a friend request from '${user.fullName}', You can accept this request by clicking here -->'https://somedia--app.herokuapp.com/users/accept/1', If you want to reject this you can click here -->'https://somedia--app.herokuapp.com/users/reject/1'.`,
        };
